...
mod tests {
    ...
    #[tokio::test]
    async fn test_no_eagain_warnings_during_queries() {
        ...
        // Create DNS client
        let client = PipelinedDnsClient::new("0.0.0.0:0", Duration::from_secs(60))
            .await
            .expect("Failed to create DNS client");

        // WHY: Query a non-routable address (TEST-NET-1, RFC 5737) with short timeout.
        // Query will timeout/fail, but we verify no EAGAIN busy-wait warnings appear.
        // This tests the fix without depending on external DNS infrastructure.
        let _ = client.query_ns("test.example", "192.0.2.1", 100).await;

        // Small delay to ensure logs are flushed
        tokio::time::sleep(Duration::from_millis(200)).await;

        // Check that no EAGAIN warnings were logged
        let captured_logs = logs.lock().unwrap();
        let eagain_warnings: Vec<_> = captured_logs
            .iter()
            .filter(|log| log.contains("Resource temporarily unavailable"))
            .map(|s| s.as_str())
            .collect();

        assert!(
            eagain_warnings.is_empty(),
            "Found EAGAIN warnings in logs (indicates busy-wait bug is back):\n{}",
            eagain_warnings.join("\n")
        );
    }
